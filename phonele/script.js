...
function compareNumeric(guessVal,targetVal){
  ...
  const level = ratio <= 0.10 ? 'near' : 'far'; // ≤10% => near
...
function autofillFromModel(parsed){
  ...
  if(found){
    ...
    parsed.battery = found.battery||'';
    parsed.year = found.year||'';
    parsed.ram = found.ram||'';
    parsed.storage = found.storage||'';
    parsed.screen = found.screen||'';
  }
}

// compute dataset stats for numeric bars
...
function renderPhonePreview(p){
  ...
  if(p.image){
    ...
    let imgPath = p.image;
    if(!imgPath.includes('/')) imgPath = `images/${String(p.brand||'').replace(/\s+/g,'')}/${imgPath}`;
...
function renderHistory(){
  ...
  state.history.slice().reverse().forEach(entry=>{
    ...
    keys.forEach(k=>{
      const chip=document.createElement('div'); chip.className='chip '+entry.result[k]; chip.textContent=(entry.guess[k]||'—');
...
function onGuess(){
  ...
  normalizeBrand(parsed); autofillFromModel(parsed);
  if(!parsed.model || !parsed.brand || parsed.model===''){
    ...
    if(foundCombined){
      ...
      parsed.battery=foundCombined.battery||''; parsed.year=foundCombined.year||''; parsed.ram=foundCombined.ram||'';
      parsed.storage=foundCombined.storage||''; parsed.screen=foundCombined.screen||'';
    }
  }
...
function attachAutocomplete(){
  ...
  input.addEventListener('keydown', e=>{
    const el=suggestionsEl; if(el.getAttribute('aria-hidden')==='true') return;
...
function newGame(){
  ...
  renderPhonePreview({model:'?',brand:'?'}); renderHistory(); $('status').textContent='Good luck!';
}

// init
async function init(){
  $('status').textContent='Loading phones.csv...';
  try{ state.phones = await loadPhonesCSV('phones.csv'); console.log('Phones loaded:',state.phones.length); }
  catch(e){ console.error('Failed to load phones.csv',e); $('status').textContent='Failed to load phones.csv'; return; }
  ...
  state.phones = state.phones.map(p=>({
    ...
    battery:Number(p.battery||0), year:Number(p.year||0), ram:Number(p.ram||0), storage:Number(p.storage||0), screen:Number(p.screen||0)
  }));

  computeStats(); buildSuggestionsIndex(); attachAutocomplete(); newGame();
